...
// Quem recebe o webhook (routes/zendesk-webhook.ts, workflows/ask-luna-workflow.ts) usa isto
// pra descobrir de qual empresa é a mensagem antes de aplicar qualquer regra de negócio.
export function resolveBusinessByAppId(appId: string): Business {
  const business = businesses.find((candidate) => candidate.channel.appId === appId);
  if (!business) {
